	...
	public function init() {
		...
		if (is_admin()) {
			...
			if ($this->active) {
				...
				add_filter('pre_option_use_balanceTags', function() { return 0; });
			}
			...
			// add jquery
			add_action('wp_loaded', function() { wp_enqueue_script('jquery'); });
			...
			$this['event']->bind('widgetoutput', function(&$content) { $content = do_shortcode($content); });
		}
    ...
    public function _adminHead() {
        ...
        if ($this->active) {
			...
			if (!is_writable($this["path"]->path("cache:"))) {
				$cachePath = $this["path"]->path("cache:");
				add_action('admin_notices', function() use ($cachePath) {
				   echo '<div class="update-nag"><strong>Widgetkit cache folder is not writable! Please check directory permissions.</strong><br />' . $cachePath . '</div>';
				    return false;
				});
			}
...
class WP_Widget_Widgetkit extends WP_Widget {
	...
	public function __construct() {
		parent::__construct('widgetkit', 'Widgetkit', array('description' => 'Display your widgets'));
...
add_action("widgets_init", function() { register_widget("WP_Widget_Widgetkit"); });
...
class WP_Widget_Widgetkit_Twitter extends WP_Widget {
	...
	public function __construct() {
		parent::__construct('widgetkit_twitter', 'Widgetkit - Twitter', array('description' => 'Lets you display your tweets'));
...
add_action("widgets_init", function() { register_widget("WP_Widget_Widgetkit_Twitter"); });
